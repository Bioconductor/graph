...
\begin{document}
\title{graphBAM and MultiGraph classes in the graph package.}
...
To do that we first obtain the from - to relationship for the \Rcode{resWt}
...
\Rfunction{nodeData} method. We would like to set the node attribute "shape" for
Seattle to the value \Rcode{"triangle"} and that for the cities that connect with Seattle to 
the value \Rcode{"circle"}.
