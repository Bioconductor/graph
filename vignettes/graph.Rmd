...
The [` r Biocpkg("graph")`](https://bioconductor.org/packages/release/bioc/html/graph.html) package provides an implementation of graphs (the kind with nodes and edges) in R. Software infrastructure is provided by three different, but related packages, 

[` r Biocpkg("graph")`](https://bioconductor.org/packages/release/bioc/html/graph.html) Provides the basic class definitions and functionality.

[` r Biocpkg("RBGL")`](https://bioconductor.org/packages/release/bioc/html/RBGL.html) Provides an interface to graph algorithms (such as shortest path, connectivity etc).

[` r Biocpkg("Rgraphviz")`](https://bioconductor.org/packages/release/bioc/html/Rgraphviz.html) Provides rendering functionality. Different layout algorithms are provided and node plotting, line type, color etc parameters can be controlled by the user.
...
The reader will benefit greatly from also having the [` r Biocpkg("Rgraphviz")`](https://bioconductor.org/packages/release/bioc/html/Rgraphviz.html) package available and from using it to
...
First we attach the [` r Biocpkg("graph")`](https://bioconductor.org/packages/release/bioc/html/graph.html) package and create a random graph (this is based on the Erdos-Renyi model for random graphs).
...
The examples here originally came from Chris Volinsky at AT&T, but have been modified in places as the [` r Biocpkg("graph")`](https://bioconductor.org/packages/release/bioc/html/graph.html) package has evolved. In the code chunk below we demonstrate how to create a graph from scratch. In this code chunk two graphs are created, gR and gR2, the first is undirected while the second is a directed graph.
...
Another useful set of graph algorithms are the so-called searching algorithm. For the [` r Biocpkg("graph")`](https://bioconductor.org/packages/release/bioc/html/graph.html) package we have implemented the depth first searching algorithm as described in Algorithm 4.2.1 of [?]. More efficient and comprehensive algorithms are available through the [` r Biocpkg("RBGL")`](https://bioconductor.org/packages/release/bioc/html/RBGL.html) package. The returned value is a named vector. The names correspond to the nodes of the graph and the values correspond to the distance (often the number of steps) or sum of the edge weights along the path to that node.
...
We provide a number of different tools that should help users coerce graphs between the different representations.
