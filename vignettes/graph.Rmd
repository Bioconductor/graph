...
The [graph](https://bioconductor.org/packages/release/bioc/html/graph.html) package provides an implementation of graphs (the kind with nodes and edges) in R. Software infrastructure is provided by three different, but related packages, 

[graph](https://bioconductor.org/packages/release/bioc/html/graph.html) Provides the basic class definitions and functionality.

[RBGL](https://bioconductor.org/packages/release/bioc/html/RBGL.html) Provides an interface to graph algorithms (such as shortest path, connectivity etc).

[Rgraphviz](https://bioconductor.org/packages/release/bioc/html/Rgraphviz.html) Provides rendering functionality. Different layout algorithms are provided and node plotting, line type, color etc parameters can be controlled by the user.
...
The reader will benefit greatly from also having the [Rgraphviz](https://bioconductor.org/packages/release/bioc/html/Rgraphviz.html) package available and from using it to
...
First we attach the [graph](https://bioconductor.org/packages/release/bioc/html/graph.html) package and create a random graph (this is based on the Erdos-Renyi model for random graphs).
...
Another useful set of graph algorithms are the so-called searching algorithm. For the [graph](https://bioconductor.org/packages/release/bioc/html/graph.html) package we have implemented the depth first searching algorithm as described in Algorithm 4.2.1 of [?]. More efficient and comprehensive algorithms are available through the [RBGL](https://bioconductor.org/packages/release/bioc/html/RBGL.html) package. The returned value is a named vector. The names correspond to the nodes of the graph and the values correspond to the distance (often the number of steps) or sum of the edge weights along the path to that node.
