...
```{r graphClassDef, warning=FALSE, message=FALSE}
library("graph")
getClass("graph")
``` 
...
In some ways it would be most natural to have `edges` methods for the `edgeSet` class the issues raised in Section [2.2.1](#sec:edgerep){reference-type="ref"reference="sec:edgerep"} seem to preclude this and it only seems to make sense to have `node` and `edges` methods for the `multiGraph` class.
